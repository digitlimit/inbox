<nav class="navbar navbar-expand-lg navbar-light bg-light mb-5">
	<div class="container-fluid">
		<a class="navbar-brand" href="{{ route(config('inbox.route.name') . 'inbox.index') }}">Messages</a>
					...
					<li class="nav-item dropdown">
						...
						<div class="dropdown-menu dropdown-menu-right">
							@foreach(auth()->user()->notifications()->latest()->get() as $notification)
								@php
									$message = \Digitlimit\Inbox\Models\Message::find($notification->data['message_id']);
									$thread = \Digitlimit\Inbox\Models\Thread::find($notification->data['thread_id']);
								@endphp
								@if($thread && $message)
